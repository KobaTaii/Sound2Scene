...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2048, pool='avgpool', zero_init_residual=False,
...
class ResNet_(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2048, pool='avgpool', zero_init_residual=False,
        ...
        if self.pool == 'avgpool':
            ...
            self.fc = nn.Linear(512 * block.expansion, num_classes)  # 8192
            self.fc2 = nn.Linear(num_classes, num_classes)  # 8192
        elif self.pool == 'vlad':
            self.avgpool = NetVLAD()
            self.fc_ = nn.Linear(8192 * block.expansion, num_classes)
...
def _resnet_(arch, block, layers, pretrained, progress, **kwargs):
...
def resnet18_(pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,**kwargs)
